...
def fetch_market_daily_data(tickers_list):
    ...
    for i in range(0, 6): # 從 0 (今天) 開始，確保 06/16 或 06/15 第一時間被掃描
        ...
        # 戰術 A：強攻證交所官方最新 RWD 歷史端點 (徹底修正舊端點死機 Bug)
        for _ in range(2):
            try:
        ...
        # 🚀 戰術 B 【同日 FinMind 攔截機制】：若官方網頁當下連線失敗或還沒結算，立刻在同一個日期鎖定 FinMind 攔截！
        if tickers_list:
            ...
            try:
                ...
                if res.json().get("msg") == "success" and len(res.json().get("data", [])) > 0:
                    # 抓到了！FinMind 在這天有真實交易數據，立刻發射信號交給迴圈進行同日 FinMind 匹配，不准再往下回溯！
                    return {"__USE_FINMIND__": True, "__TARGET_DATE__": target_date_fm}
            except: pass

    return {}

# --- 3. 網頁視覺化側邊欄與標題區 (完美融合模板樣式) ---
# 側邊欄控制
...
if not df_display.empty:
    ...
    st.info("📭 目前清單為空。請點擊左側「🧹 清除網頁快取」後重整。")
